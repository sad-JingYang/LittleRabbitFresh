...
const GetMemberOrderById = async () => {
  ...
  if (
    ...
    await GetMemberOrderLogisticsById()
  }
}
...
const onOrderPay = async () => {
  ...
  if (import.meta.env.DEV) {
    ...
    await FetchPayMock({ orderId: query.id })
  } else {
    // 生产环境：获取支付参数 + 发起微信支付
    const res = await FetchPayWxPayMiniPay({ orderId: query.id })
...
const GetMemberOrderLogisticsById = async () => {
  const res = await FetchMemberOrderLogisticsById(query.id)
...
const onOrderDelete = () => {
  ...
  uni.showModal({
    ...
    success: async (success) => {
      if (success.confirm) {
        await deleteMemberOrder({ ids: [query.id] })
        await uni.redirectTo({ url: '/pagesOrder/list/list' })
      }
    },
  })
}
